...
def run_test_suite(test_file: str, description: str) -> dict:
    ...
    with Progress(
        ...
        try:
            ...
            # Display results
            if passed:
                console.print(f"✅ [green]{description}[/green] ([#0066CC]{duration:.1f}s[/#0066CC])")
                if parsed['tests']:
                    for test in parsed['tests']:
                        if test['status'] == 'PASSED':
                            console.print(f"   • [green]{test['name']}[/green]")
            else:
                console.print(f"❌ [red]{description}[/red] ([#0066CC]{duration:.1f}s[/#0066CC])")
            ...
            progress.update(task, completed=100)
            duration = time.time() - start_time
            console.print(f"💥 [red]{description} ERROR[/red] ([#0066CC]{duration:.1f}s[/#0066CC]): {e}")
            return {
                "name": description,
                "file": test_file,
                "passed": False,
...
def main():
    ...
    table.add_column("Metric", style="#0066CC", width=20)
